...
const StoreContextProvider = (props) => {
    ...
    const url = 'https://backend-psbv.onrender.com'


    //sending a api request to get the available food items from the database
    const [product_list, setPoduct_List] = useState([])

    const getfood = async () => {
      const response = await axios.get(`${url}/api/food/list`)
      if (response.data.success) {
        setPoduct_List(response.data.data)
      } else {
    ...
    const getTotalAmount = () => {
        ...
        for (const item in cartItem) {
            let price = product_list.find((food) => food._id === item)
